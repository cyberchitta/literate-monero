...
- Same infrastructure works for validation-only (mining can be disabled later)
...
#+begin_src yaml :tangle ../ansible/fragments/07-mining.yml
    # ------------------------------------------------------------------------
    # Monero Mining Setup
    # ------------------------------------------------------------------------
...
#+begin_src yaml :tangle ../ansible/fragments/07-mining.yml
    ...
    - name: Configure XMRig build
      command:
        ...
        creates: /opt/xmrig/build/Makefile
      tags: [mining]
...
** XMRig MSR Access Configuration

XMRig requires access to Model-Specific Registers (MSR) for optimal RandomX mining performance.
...
**Security consideration:** MSR access allows low-level CPU control. The mining user is granted this via udev rules and Linux capabilities, not via sudo/root access.
...
#+begin_src yaml :tangle ../ansible/fragments/07-mining.yml
    ...
    - name: Extract P2Pool
      unarchive:
        ...
        dest: "{{ p2pool_data_dir }}"
        remote_src: yes
        extra_opts: [--strip-components=1]
      when: p2pool_download.changed
      register: p2pool_extract
      tags: [mining, p2pool]

    - name: Check P2Pool binary exists
      stat:
        path: "{{ p2pool_data_dir }}/p2pool"
      register: p2pool_binary
      tags: [mining, p2pool]
    ...
    - name: Fail if P2Pool not found
      ...
      when: not p2pool_binary.stat.exists
      tags: [mining, p2pool]
...
User={{ mining_user }}
Group={{ mining_user }}
WorkingDirectory={{ p2pool_data_dir }}

# P2Pool always uses local monerod (core infrastructure)
ExecStart={{ p2pool_data_dir }}/p2pool \
  --host 127.0.0.1 \
...
#+begin_src yaml :tangle ../ansible/fragments/07-mining.yml
    ...
    - name: Verify P2Pool mode matches monerod setting
      assert:
        that:
          - (monerod_enabled | default(false) and p2pool_mode.stdout == "full_node") or (not (monerod_enabled | default(false)) and p2pool_mode.stdout == "light_client")
        fail_msg: |
          P2Pool configuration mismatch!
          monerod_enabled: {{ monerod_enabled | default(false) }}
          P2Pool mode: {{ p2pool_mode.stdout }}
          Expected: {{ 'full_node' if monerod_enabled | default(false) else 'light_client' }}
        success_msg: "✓ P2Pool using correct mode: {{ p2pool_mode.stdout }}"
      tags: [mining, verify]

    - name: Test monerod is responsive (if full node mode)
      uri:
      ...
      failed_when: false
      when: monerod_enabled | default(false)
      tags: [mining, verify]

    - name: Warn if monerod not responding
      debug:
        msg:
          - "⚠ WARNING: monerod RPC not responding"
          - "P2Pool is configured for full node but can't reach monerod"
          - "P2Pool will wait for monerod to sync before starting mining"
          - "Check: sudo journalctl -u monerod -f"
      when:
        - monerod_enabled | default(false)
        - monerod_test.status != 200
      tags: [mining, verify]
    ...
    - name: Display mining status
      debug:
        msg:
          ...
          - ""
          - "{% if monerod_enabled | default(false) %}P2Pool using local monerod (maximum privacy){% else %}P2Pool using light client (minimal resources){% endif %}"
          - ""
          - "Monitor mining:"
          - "  sudo journalctl -u xmrig -f"
          - "  sudo mining-monitor.sh"
          - ""
          - "{% if monerod_enabled | default(false) %}Monitor monerod sync:{% endif %}"
          - "{% if monerod_enabled | default(false) %}  sudo monerod-status.sh{% endif %}"
          - "{% if monerod_enabled | default(false) %}  sudo journalctl -u monerod -f{% endif %}"
      tags: [mining, verify]
#+end_src
...
*P2Pool Types*:
- normal: Main pool (hashrate > 100 kH/s)
- mini: For smaller miners (< 100 kH/s) - recommended for most home systems
- nano: For very low hashrate (experimental)

Set `p2pool_type` in config.yml.

*** P2Pool and Monerod Sync

**During monerod's initial sync (6-24 hours):**

P2Pool and XMRig will start successfully, but:
- XMRig shows "connected" to P2Pool ✓
- Hashrate appears in logs ✓
- **No shares found/accepted** (expected)

This is normal. P2Pool waits for monerod to finish syncing before participating in mining.

Monitor sync progress:
#+begin_example
# Check if monerod is still syncing
sudo monerod-status.sh

# Watch for: "✓ Blockchain synced"
# Once synced, you'll see "share found" in P2Pool logs

# Monitor P2Pool
sudo journalctl -u p2pool -f
#+end_example

**Making mining optional later:**

If you want validation-only (no mining), this can be made conditional:
1. Add `mining_enabled: false` to config.yml
2. Add `when:` guards to mining phase tasks
3. Re-run playbook

For now, mining is enabled to demonstrate the full stack.

*** P2Pool Network Requirements

P2Pool requires port {{ p2pool_p2p_port }} open for incoming connections:
- Allows peer-to-peer pool communication
- Contributes to pool decentralization
- Mining works without this, but you won't help other miners

The port depends on pool type:
- normal: 37889
- mini: 37888  
- nano: 37890

If behind a router, forward this port (TCP+UDP) to your server.

The firewall rules are configured in the firewall phase when UFW is enabled.
