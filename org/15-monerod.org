...
The release tarball's hash is checked against =hashes.txt= from
getmonero.org, and =hashes.txt= itself is verified against binaryFate's
release-signing key — pinned in =keys/binaryfate.asc= (see
=keys/README.md= for fingerprint and provenance). Without the signature
check, whoever can tamper with the download can tamper with the hash
list too.
...
#+begin_src yaml :tangle ../ansible/fragments/15-monerod.yml
    ...
    - name: Copy pinned Monero release-signing key (binaryFate)
      copy:
        src: ../keys/binaryfate.asc
        dest: "{{ gpg_verify_home }}/binaryfate.asc"
        owner: root
        group: root
        mode: '0644'
      tags: [monerod]

    - name: Import Monero release-signing key into verification keyring
      command: gpg --homedir {{ gpg_verify_home }} --import {{ gpg_verify_home }}/binaryfate.asc
      register: binaryfate_import
      changed_when: "'imported: 1' in binaryfate_import.stderr"
      tags: [monerod]
    ...
    - name: Verify hashes.txt signature (binaryFate)
      command: gpg --homedir {{ gpg_verify_home }} --verify /tmp/monero-hashes.txt
      changed_when: false
      when: not monerod_version_exists.stat.exists
      tags: [monerod]
    ...
    - name: Fail if expected hash is not a sha256 hex string
      fail:
        msg: |
          Could not extract a valid sha256 for monero-linux-x64-{{ monerod_version }}.tar.bz2
          from hashes.txt (got: '{{ expected_hash.stdout }}').
          hashes.txt only lists the current release - if {{ monerod_version }}
          is no longer current, update monerod_version in config.yml.
      when:
        - not monerod_version_exists.stat.exists
        - expected_hash.stdout is not match('^[a-f0-9]{64}$')
      tags: [monerod]
...
monerod is internet-facing (P2P on 0.0.0.0, plus I2P inbound), so it runs as a
dedicated unprivileged system user rather than root — an RCE in the daemon is
then contained to that account instead of being instant root.

#+begin_src yaml :tangle ../ansible/fragments/15-monerod.yml
    - name: Create monero service user (no login shell)
      user:
        name: monero
        system: yes
        shell: /usr/sbin/nologin
        create_home: no
        home: "{{ monerod_data_dir }}"
      tags: [monerod]
#+end_src
...
The list is GPG-verified against Boog900's maintainer key, pinned in
=keys/boog900.asc= (see =keys/README.md= — the pinned key is *not*
fetched from the same host as the list, which is what previously made
verification circular). Initial download and weekly updates share the
same script, so both paths get signature + format checks.

#+begin_src yaml :tangle ../ansible/fragments/15-monerod.yml
    - name: Copy pinned ban-list signing key (Boog900)
      copy:
        src: ../keys/boog900.asc
        dest: "{{ gpg_verify_home }}/boog900.asc"
        owner: root
        group: root
        mode: '0644'
      tags: [monerod, config, security]

    - name: Import ban-list signing key into verification keyring
      command: gpg --homedir {{ gpg_verify_home }} --import {{ gpg_verify_home }}/boog900.asc
      register: boog900_import
      changed_when: "'imported: 1' in boog900_import.stderr"
      tags: [monerod, config, security]
    ...
    - name: Deploy ban list update script
      template:
        ...
        mode: '0755'
      tags: [monerod, config, scripts, security]

    - name: Download and verify initial ban list
      command: /usr/local/bin/update-monero-banlist.sh
      args:
        creates: /etc/monero/ban_list.txt
      tags: [monerod, config, security]
    ...
    - name: Display ban list stats
      debug:
        msg:
          ...
          - "GPG-verified against pinned key keys/boog900.asc"
      tags: [monerod, config, security]
#+end_src
...
{% if monerod_rpc_bind_ip != '127.0.0.1' %}
# Required acknowledgement for binding RPC to a non-loopback address
...
#+begin_src yaml :tangle ../ansible/fragments/15-monerod.yml
    ...
    - name: Deploy monerod launch wrapper
      template:
        src: ../configs/templates/monerod-launch.sh.j2
        dest: /usr/local/bin/monerod-launch.sh
        mode: '0755'
        owner: root
        group: root
      tags: [monerod, config]
...
** monerod Launch Wrapper

Reads the I2P address written by the wait script, re-validates it against the
strict b32 format, and execs monerod with the address as a single quoted
argument. This replaces a previous =/bin/sh -c '... $(cat ...) ...'= ExecStart,
which substituted file contents straight into the command line (argument/flag
injection if the file were ever tampered with).

#+begin_src bash :tangle ../configs/templates/monerod-launch.sh.j2
#!/bin/bash
# monerod launch wrapper — see org/15-monerod.org
set -euo pipefail

ADDR=$(cat /etc/monero/i2p-address.txt)
if ! [[ "$ADDR" =~ ^[a-z0-9]{52}\.b32\.i2p$ ]]; then
  echo "ERROR: invalid I2P address in /etc/monero/i2p-address.txt" >&2
  exit 1
fi

exec {{ monerod_install_dir }}/monerod \
  --config-file /etc/monerod.conf \
  --tx-proxy i2p,127.0.0.1:7656 \
  --anonymous-inbound "${ADDR},127.0.0.1:18085,128" \
  --non-interactive
#+end_src
...
# Wait for I2P tunnel to be ready before starting. The '+' runs this as root
# (not the monero user) so it can write the address into root-owned /etc/monero,
# which the service user is intentionally not allowed to modify.
ExecStartPre=+/usr/local/bin/wait-for-i2p-tunnel.sh

# The launch wrapper reads the I2P address from /etc/monero/i2p-address.txt,
# re-validates it against the strict b32 format, and passes it as a single
# quoted argv element — no shell command-substitution into the command line.
ExecStart=/usr/local/bin/monerod-launch.sh
...
ProtectSystem=full
ProtectHome=true
# Only the data dir is writable; /etc/monero stays read-only to the daemon so a
# compromised monerod cannot rewrite its own ban list or announced I2P address.
ReadWritePaths={{ monerod_data_dir }}
...
#!/bin/bash
# Ban list update (see the monerod phase org file)
# Downloads to a private mktemp dir, verifies Boog900's detached GPG
# signature against the pinned key in {{ gpg_verify_home }}, and sanity-
# checks the format before installing.

set -euo pipefail

BAN_LIST="/etc/monero/ban_list.txt"
BAN_LIST_URL="https://raw.githubusercontent.com/Boog900/monero-ban-list/refs/heads/main/ban_list.txt"
SIG_URL="https://raw.githubusercontent.com/Boog900/monero-ban-list/refs/heads/main/sigs/boog900.sig"
GPG_HOME="{{ gpg_verify_home }}"

TMP_DIR=$(mktemp -d)
trap 'rm -rf "$TMP_DIR"' EXIT
TEMP_FILE="$TMP_DIR/ban_list.txt"

curl -fsS "$BAN_LIST_URL" -o "$TEMP_FILE"
curl -fsS "$SIG_URL" -o "$TMP_DIR/boog900.sig"

if ! gpg --homedir "$GPG_HOME" --verify "$TMP_DIR/boog900.sig" "$TEMP_FILE" 2>/dev/null; then
  echo "ERROR: ban list signature verification failed" >&2
  exit 1
fi

# Only comments, blank lines and IPv4[/prefix] entries are expected;
# anything else (HTML error page, new format) fails closed for review.
if grep -vE '^(#.*)?$|^[0-9]{1,3}(\.[0-9]{1,3}){3}(/[0-9]{1,2})?$' "$TEMP_FILE" | grep -q .; then
  echo "ERROR: ban list contains unexpected lines:" >&2
  grep -vE '^(#.*)?$|^[0-9]{1,3}(\.[0-9]{1,3}){3}(/[0-9]{1,2})?$' "$TEMP_FILE" | head -5 >&2
  exit 1
fi
...
Set up weekly updates (the script itself is deployed earlier, before the
initial verified download).
