...
- **First launch**: Opens Tor Browser with github.com in dedicated profile
- **Subsequent launches**: Focuses existing Tor Browser window
- **Session persistence**: Keep browser running = stay logged in, close browser = log out
- **Shared profile**: Other apps (Element, etc.) can use same profile and session

**** Adding More Apps

To add apps that authenticate via GitHub (e.g., Element):

#+begin_example
# Create app script
cat > configs/templates/element-browser.sh.j2 << 'EOF'
#!/bin/bash
/usr/local/bin/github-auth-browser.sh https://app.element.io own-window "Element"
EOF

# Add deployment task in org/03-privacy.org
# Add desktop entry
#+end_example

The "own-window" mode ensures Element gets its own window, separate from GitHub.

**** Security Model

- All apps share same Tor circuit (exit node)
- All apps share GitHub authentication
- Standard Tor Browser security (no session persistence on close)
- Close browser to clear all sessions and log out
#+end_src
