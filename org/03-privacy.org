...
**Purpose:** These keys authenticate git push/pull operations to GitHub/GitLab through Tor.

**Different from server access keys:** Phase 2 provisioned keys for SSH login (laptop → server). These are for git operations (server → GitHub/GitLab via Tor).

#+begin_src bash :tangle ../configs/templates/generate-git-keys.sh.j2
#!/bin/bash
# Generate SSH keys for git hosting services
# These keys are used for: git push/pull to GitHub/GitLab (routed through Tor)
# NOT used for: SSH login to this server (those keys in Phase 2)
