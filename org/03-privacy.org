...
def main() -> None:
    ...
    if domain in kyc_domains:
        # KYC identity sites → Brave with KYCIdentity profile
        import os
        os.execvp("brave", ["brave", "--profile-directory=KYCIdentity", url])
    else:
    ...
    - name: Display browser dispatcher status
      debug:
        msg:
          ...
          - "  - KYC domains → Brave with KYCIdentity profile"
          - "  - All other URLs → Tor Browser"
          - ""
          - "Brave profiles auto-create on first use (no manual setup needed)"
          - ""
      tags: [privacy, browser-dispatch, verify]
#+end_src
