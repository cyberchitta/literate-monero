...
# Backs up critical data to {{ backup_dir }}
...
**SSH Keys (from wireguard and privacy phases):**
- Server access keys: `/home/dev/.ssh/authorized_keys`
- Git hosting keys: `/home/dev/.ssh/id_ed25519_github`, `id_ed25519_gitlab`
- SSH config for Tor routing: `/home/dev/.ssh/config`

**Note:** All SSH keys in `/home/dev/.ssh` are backed up, including both server access and git hosting keys.

**GPG Keys:**
- `/home/dev/.gnupg` - All GPG keys and configuration

**Mining Configuration:**
- XMRig config: `/opt/xmrig/build/config.json`
- Mining data: `/opt/mining`

**Tails VM:**
- Persistent storage disk: `/var/lib/libvirt/images/tails-fortress-persistent.qcow2` (~8GB)
- Contains: crypto wallets, KeePassXC database, browser bookmarks, GnuPG keys

**System Configuration:**
- All of `/etc` (systemd services, configs, etc.)

**What is NOT backed up:**
- XMRig binary (rebuilt from source during restore)
- P2Pool binary (re-downloaded during restore)
- Tails ISO (re-downloaded during restore)
- monerod blockchain (~160GB - too large, re-syncs during restore)
- Temporary files matching patterns: {{ backup_exclude_patterns | join(', ') }}
...
- Mining: < 100 MB
- Tails VM: ~8 GB (persistent storage)
- System config: < 100 MB

Total: 10-15 GB per backup
