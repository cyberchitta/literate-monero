...
This script manages Mullvad VPN connection for browser sessions. Used by both browser dispatcher (automatic routing) and desktop launcher (manual fallback).

#+begin_src bash :tangle ../configs/templates/mullvad-browser.sh.j2
#!/bin/bash
...
# Check if Mullvad config exists
if ! [ -f /etc/wireguard/wg-mullvad.conf ]; then
  echo "Error: Mullvad not configured"
  echo "See Phase 11 documentation for setup"
  exit 1
fi

# Enable wg-mullvad if not already up
CLEANUP_NEEDED=false
if ! wg show wg-mullvad &>/dev/null 2>&1; then
  sudo wg-quick up wg-mullvad
  CLEANUP_NEEDED=true
  sleep 2
fi

# Launch Chromium with mullvad-fallback profile
chromium --profile-directory=mullvad-fallback "$@" &
BROWSER_PID=$!

# Wait for browser to exit
wait $BROWSER_PID

# Cleanup: Disable wg-mullvad if we enabled it
...
#+begin_src yaml :tangle ../ansible/fragments/12-privacy.yml
    ...
    - name: Deploy Mullvad browser wrapper
      template:
        src: ../configs/templates/mullvad-browser.sh.j2
