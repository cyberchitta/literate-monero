...
#!/usr/bin/env python3
"""Browser dispatcher with profile-based domain routing for privacy-first browsing."""
import sys
import json
import os
import subprocess
...
def focus_tor_browser() -> bool:
    """Try to focus existing Tor Browser window. Returns True if found."""
    try:
...
def main() -> None:
    ...
    config = load_config()
    # Check each profile for matching domain
    for profile in config.get('browser_profiles', []):
        ...
        if domain in profile_domains:
            # Look up directory name from display name
            profile_dir = get_profile_directory(profile_name)
            if profile_dir:
                os.execvp("chromium", ["chromium", f"--profile-directory={profile_dir}", url])
            else:
                # Profile not found, fall through to Tor Browser
                break
    # No match found - default to Tor Browser (privacy-first)
    # Try to focus existing window, otherwise launch new instance
    if not focus_tor_browser():
        os.execvp("tor-browser", ["tor-browser"])
    else:
        # Window focused, exit successfully
        sys.exit(0)
    ...
    - name: Note - Install git-credential-keepassxc from AUR
      debug:
        msg: |
          ...
          yay -S --noconfirm git-credential-keepassxc-git
          (or paru -S --noconfirm git-credential-keepassxc-git)
