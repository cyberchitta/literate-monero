...
def main():
    # Check if wg-mullvad is already up
    result = subprocess.run(
        ["sudo", "wg", "show", "wg-mullvad"],
        capture_output=True
    )
    
    cleanup_needed = (result.returncode != 0)
    
    if cleanup_needed:
        # Enable wg-mullvad
        subprocess.run(["sudo", "wg-quick", "up", "wg-mullvad"], check=True)
        import time
        time.sleep(2)
    ...
    if cleanup_needed:
        subprocess.run(["sudo", "wg-quick", "down", "wg-mullvad"])
