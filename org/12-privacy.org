...
#+begin_src yaml :tangle ../ansible/fragments/12-privacy.yml
    # ------------------------------------------------------------------------
...
#+begin_src yaml :tangle ../ansible/fragments/12-privacy.yml
    - name: Check I2P web console accessibility
      ...
      failed_when: false
      tags: [privacy, i2p, verify, config]
...
As the first phase to verify a release signature, privacy creates the
shared GPG verification keyring directory ([[file:00-configuration.org][gpg_verify_home]]). The later
phases (monerod, wallet, mining) import their pinned keys into this
same keyring and rely on this phase having created it.
...
#+begin_src yaml :tangle ../ansible/fragments/12-privacy.yml
    ...
    - name: Copy pinned Tor Browser signing key
      copy:
        src: ../keys/torbrowser.asc
        dest: "{{ gpg_verify_home }}/torbrowser.asc"
        owner: root
        group: root
        mode: '0644'
      tags: [privacy, browser]

    - name: Import Tor Browser signing key into verification keyring
      command: gpg --homedir {{ gpg_verify_home }} --import {{ gpg_verify_home }}/torbrowser.asc
      register: torbrowser_import
      changed_when: "'imported: 1' in torbrowser_import.stderr"
      tags: [privacy, browser]
    ...
    - name: Download Tor Browser checksums signature
      get_url:
        url: "https://dist.torproject.org/torbrowser/{{ tor_browser_version }}/sha256sums-signed-build.txt.asc"
        dest: /tmp/tor-browser-checksums.txt.asc
        mode: '0644'
      when: not tor_browser_check.stat.exists
      tags: [privacy, browser]

    - name: Verify Tor Browser checksums signature
      command: gpg --homedir {{ gpg_verify_home }} --verify /tmp/tor-browser-checksums.txt.asc /tmp/tor-browser-checksums.txt
      changed_when: false
      when: not tor_browser_check.stat.exists
      tags: [privacy, browser]
    ...
    - name: Fail if Tor Browser checksum is not a sha256 hex string
      fail:
        msg: |
          Could not extract a valid sha256 for tor-browser-linux-x86_64-{{ tor_browser_version }}.tar.xz
          (got: '{{ tor_browser_checksum.stdout }}').
          dist.torproject.org removes old releases - if {{ tor_browser_version }}
          is no longer current, update tor_browser_version in config.yml.
      when:
        - not tor_browser_check.stat.exists
        - tor_browser_checksum.stdout is not match('^[a-f0-9]{64}$')
      tags: [privacy, browser]
...
def get_profile_directory(profile_name: str) -> str:
    """Map profile display name to directory name via Local State."""
...
def focus_tor_browser() -> bool:
    try:
        ...
        if result.returncode == 0:
            ...
            clients = json.loads(windows.stdout)
            for client in clients:
                if "tor browser" in client.get("class", "").lower() or \
                   "tor browser" in client.get("title", "").lower():
...
def main() -> None:
    ...
    config = load_config()
    for profile in config.get('browser_profiles', []):
        profile_name = profile.get('name', '')
        profile_domains = {d.lower() for d in profile.get('domains', [])}
        if domain in profile_domains:
            if profile_name == 'mullvad-fallback':
                os.execvp("mullvad-browser.sh", ["mullvad-browser.sh", url])
            else:
                profile_dir = get_profile_directory(profile_name)
                if profile_dir:
                    os.execvp("chromium", ["chromium", f"--profile-directory={profile_dir}", url])
                else:
                    break
    if not focus_tor_browser():
        os.execvp("tor-browser", ["tor-browser"])
    else:
        sys.exit(0)
...
The wrapper refuses to launch the browser unless the tunnel is verifiably
up: =wg-quick up= must succeed AND Mullvad's own check service
(=am.i.mullvad.net=) must confirm the exit IP is a Mullvad relay. While
the browser runs, a watchdog polls the tunnel interface and kills the
browser the moment it disappears, so a dropped tunnel cannot silently
fail open to the clear internet.

A host-wide default-deny egress firewall for the browser's lifetime was
considered and rejected: this machine also runs monerod, Tor, I2P and
P2Pool, and a global egress block would sever them. The
verify-then-watchdog approach scopes the protection to the browser
without touching node traffic.

#+begin_src python :tangle ../configs/templates/mullvad-browser.py.j2
#!/usr/bin/env python3
# Launch browser with Mullvad VPN
# Generated from org/12-privacy.org
# Used by: browser dispatcher + desktop launcher (DRY)
#
# Kill-switch behavior:
#   - refuses to launch unless wg-quick up succeeds and am.i.mullvad.net
#     confirms a Mullvad exit IP
#   - watchdog kills the browser if the tunnel interface disappears
...
WG_INTERFACE = "wg-mullvad"
# Optional override of which Chromium profile to open, as a literal
# --profile-directory name. The interop phase's `rabby-wallet` launcher sets it
# to its dedicated wallet profile, reusing this fail-closed path instead of
# duplicating it. Unset (the normal case) → the mullvad-fallback profile,
# resolved by display name exactly as before.
BROWSER_PROFILE = os.environ.get("MULLVAD_BROWSER_PROFILE")
...
def verify_mullvad_exit(retries: int = 3, delay: float = 2.0) -> bool:
    """Confirm traffic actually exits via a Mullvad relay."""
    for _ in range(retries):
        try:
            with urllib.request.urlopen(MULLVAD_CHECK_URL, timeout=10) as resp:
...
def tunnel_watchdog(browser_proc: subprocess.Popen):
    """Kill the browser if the tunnel drops - never fail open."""
    while browser_proc.poll() is None:
        if not tunnel_is_up():
            print("mullvad-browser: tunnel dropped - killing browser",
                  file=sys.stderr)
            browser_proc.kill()
            return
...
def main():
    cleanup_needed = not tunnel_is_up()
    ...
    if cleanup_needed:
        ...
        if up.returncode != 0:
            print(f"mullvad-browser: failed to bring up {WG_INTERFACE}:",
                  file=sys.stderr)
            print(up.stderr, file=sys.stderr)
            sys.exit(1)

    if not verify_mullvad_exit():
        print("mullvad-browser: exit IP is NOT a Mullvad relay - refusing "
              "to launch browser", file=sys.stderr)
        if cleanup_needed:
            subprocess.run(["sudo", "wg-quick", "down", WG_INTERFACE],
                           capture_output=True)
        sys.exit(1)

    # Get profile directory. An explicit override is used as a literal directory
    # (deterministic path, e.g. for the interop wallet); otherwise resolve the
    # mullvad-fallback profile's directory by its display name.
    profile_dir = BROWSER_PROFILE or get_profile_directory("mullvad-fallback")

    # Launch Chromium
    args = ["chromium", f"--profile-directory={profile_dir}"] + sys.argv[1:]
    browser_proc = subprocess.Popen(args)

    watchdog = threading.Thread(target=tunnel_watchdog, args=(browser_proc,),
                                daemon=True)
    watchdog.start()
    ...
    # Cleanup if we enabled wg-mullvad
    if cleanup_needed:
        subprocess.run(["sudo", "wg-quick", "down", WG_INTERFACE])
...
Provides GUI icon for manual Tor fallback.

#+begin_src conf :tangle ../configs/templates/mullvad-browser.desktop.j2
[Desktop Entry]
Version=1.0
Type=Application
Name=Mullvad Browser
GenericName=Web Browser (Mullvad VPN)
Comment=Browse with Mullvad VPN (Tor fallback)
Exec=/usr/local/bin/mullvad-browser.sh
Icon=web-browser
Terminal=false
Categories=Network;WebBrowser;
StartupNotify=true
...
*** Deploy Mullvad Browser Integration
...
#+begin_src yaml :tangle ../ansible/fragments/12-privacy.yml
    ...
    - name: Deploy Mullvad browser wrapper
      template:
        src: ../configs/templates/mullvad-browser.py.j2
        dest: /usr/local/bin/mullvad-browser.sh
        mode: '0755'
      tags: [privacy, mullvad, browser]

    - name: Deploy Mullvad browser desktop entry
      template:
        src: ../configs/templates/mullvad-browser.desktop.j2
        dest: /usr/share/applications/mullvad-browser.desktop
        mode: '0644'
      tags: [privacy, mullvad, browser]

    - name: Configure sudo access for Mullvad VPN control
      copy:
        content: |
          # Allow dev user to control Mullvad VPN without password
          {{ dev_user }} ALL=(ALL) NOPASSWD: /usr/bin/wg-quick up wg-mullvad
          {{ dev_user }} ALL=(ALL) NOPASSWD: /usr/bin/wg-quick down wg-mullvad
          {{ dev_user }} ALL=(ALL) NOPASSWD: /usr/bin/wg show wg-mullvad
        dest: /etc/sudoers.d/mullvad-vpn
        mode: '0440'
        validate: 'visudo -cf %s'
      tags: [privacy, mullvad, security]

    - name: Verify Mullvad browser integration
      stat:
        path: /usr/local/bin/mullvad-browser.sh
      register: mullvad_browser_installed
      tags: [privacy, mullvad, verify]

    - name: Display Mullvad browser status
      debug:
        msg:
          - "=== Mullvad Browser Integration ==="
          - ""
          - "Wrapper: {% if mullvad_browser_installed.stat.exists %}Installed{% else %}Not found{% endif %}"
          - "Desktop launcher: /usr/share/applications/mullvad-browser.desktop"
          - ""
          - "Usage:"
          - "  1. Automatic: Configure domains in browser_profiles (Phase 13)"
          - "  2. Manual: Click 'Mullvad Browser' icon when Tor blocked"
          - ""
          - "Behavior:"
          - "  - Enables wg-mullvad on launch"
          - "  - Opens Chromium (mullvad-fallback profile)"
          - "  - Disables wg-mullvad on exit"
          - ""
          - "Mullvad config status: {% if mullvad_config.stat.exists %}Configured{% else %}Not configured (see Phase 11){% endif %}"
          - ""
      tags: [privacy, mullvad, verify]
#+end_src
...
*** Secure Git Credential Storage with KeePassXC

Store GitHub PAT in KeePassXC.

**** Install git-credential-keepassxc helper (AUR)

#+begin_src yaml :tangle ../ansible/fragments/12-privacy.yml
    - name: Note - Install git-credential-keepassxc from AUR
      debug:
        msg: |
          One-time manual step (run as {{ dev_user }}):
          yay -S --noconfirm git-credential-keepassxc
          (or paru -S --noconfirm git-credential-keepassxc)
      tags: [privacy, keepassxc, git]
#+end_src

**** Configure git credential helper

#+begin_src yaml :tangle ../ansible/fragments/12-privacy.yml
    - name: Set git to use KeePassXC credential helper
      become: yes
      become_user: "{{ dev_user }}"
      command: git config --global credential.helper '!git-credential-keepassxc'
      changed_when: false
      tags: [privacy, git, keepassxc]
#+end_src

**** KeePassXC usage for git

- Unlock KeePassXC once after login (manual master password).
- Create entry: Title "GitHub PAT", Username = github username, Password = classic PAT (repo scope).
- First git push/fetch triggers KeePassXC popup → Allow (or Allow always).
- No further typing needed during the session.

Security: PAT stays encrypted in KeePassXC database.
