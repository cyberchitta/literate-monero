...
#+begin_src jinja2 :tangle ../configs/templates/xmrig-config.json.j2
{
    ...
    "randomx": {
        ...
        "mode": "auto",
        "1gb-pages": false,
        "rdmsr": {{ mining_msr_access | lower }},
        "wrmsr": {{ mining_msr_access | lower }},
        "cache_qos": false,
        "numa": true,
...
#+begin_src yaml :tangle ../ansible/fragments/19-mining.yml
    # ------------------------------------------------------------------------
    # Monero Mining Setup
    # ------------------------------------------------------------------------
    ...
    # MSR access is opt-in (mining_msr_access in config.yml): it buys
    # ~5-15% hashrate at the cost of giving the miner hardware-level
    # register access. Disabled config also removes prior grants.
    # udev only reads files ending in .rules; an earlier revision
    # shipped this as 99-msr.conf, which udev silently ignored.
    - name: Remove misnamed MSR udev rule
      file:
        path: /etc/udev/rules.d/99-msr.conf
        state: absent
      tags: [mining]
    ...
    - name: Create udev rule for MSR device access
      copy:
        ...
        dest: /etc/udev/rules.d/99-msr.rules
        mode: '0644'
      when: mining_msr_access | bool
      tags: [mining]
    ...
    - name: Remove MSR udev rule (MSR access disabled)
      file:
        path: /etc/udev/rules.d/99-msr.rules
        state: absent
      when: not (mining_msr_access | bool)
      tags: [mining]

    - name: Remove MSR module autoload (MSR access disabled)
      file:
...
#+begin_src yaml :tangle ../ansible/fragments/19-mining.yml
    ...
    # Ownership must be settled BEFORE granting capabilities:
    # chown clears file capabilities, so a fresh build (root-owned)
    # would lose cap_sys_rawio the moment it is chowned.
    ...
    # The capabilities module's state=absent generates a setcap
    # invocation that current libcap rejects; use setcap -r instead.
    - name: Check XMRig file capabilities
      command: getcap {{ xmrig_versioned_dir }}/build/xmrig
      register: xmrig_caps
      changed_when: false
      when: not (mining_msr_access | bool)
      tags: [mining]

    - name: Remove XMRig MSR capability (MSR access disabled)
      command: setcap -r {{ xmrig_versioned_dir }}/build/xmrig
      when:
        - not (mining_msr_access | bool)
        - "'cap_sys_rawio' in xmrig_caps.stdout"
      tags: [mining]
...
P2Pool releases ship a =sha256sums.txt.asc= signed by SChernykh; the
tarball hash is checked against it after verifying the signature with
the key pinned in =keys/schernykh.asc= (see =keys/README.md=).
...
#+begin_src yaml :tangle ../ansible/fragments/19-mining.yml
    ...
    - name: Copy pinned P2Pool release-signing key (SChernykh)
      copy:
        src: ../keys/schernykh.asc
        dest: "{{ gpg_verify_home }}/schernykh.asc"
        owner: root
        group: root
        mode: '0644'
      tags: [mining, p2pool]

    - name: Import P2Pool release-signing key into verification keyring
      command: gpg --homedir {{ gpg_verify_home }} --import {{ gpg_verify_home }}/schernykh.asc
      register: schernykh_import
      changed_when: "'imported: 1' in schernykh_import.stderr"
      tags: [mining, p2pool]
    ...
    - name: Download P2Pool signed checksums
      get_url:
        url: "https://github.com/SChernykh/p2pool/releases/download/{{ p2pool_version }}/sha256sums.txt.asc"
        dest: /tmp/p2pool-sha256sums.txt.asc
        mode: '0644'
      when: not p2pool_version_exists.stat.exists
      tags: [mining, p2pool]

    - name: Verify P2Pool checksums signature (SChernykh)
      command: gpg --homedir {{ gpg_verify_home }} --verify /tmp/p2pool-sha256sums.txt.asc
      changed_when: false
      when: not p2pool_version_exists.stat.exists
      tags: [mining, p2pool]

    - name: Extract expected hash for P2Pool tarball
      shell: |
        awk -v f="p2pool-{{ p2pool_version }}-linux-x64.tar.gz" \
          '$1=="Name:" && $2==f {found=1} found && $1=="SHA256:" {print $2; exit}' \
          /tmp/p2pool-sha256sums.txt.asc
      register: p2pool_expected_hash
      changed_when: false
      when: not p2pool_version_exists.stat.exists
      tags: [mining, p2pool]

    - name: Fail if expected P2Pool hash is not a sha256 hex string
      fail:
        msg: |
          Could not extract a valid sha256 for p2pool-{{ p2pool_version }}-linux-x64.tar.gz
          from sha256sums.txt.asc (got: '{{ p2pool_expected_hash.stdout }}').
      when:
        - not p2pool_version_exists.stat.exists
        - p2pool_expected_hash.stdout is not match('^[a-f0-9]{64}$')
      tags: [mining, p2pool]

    - name: Compute P2Pool download hash
      stat:
        path: /tmp/p2pool.tar.gz
        checksum_algorithm: sha256
      register: p2pool_download_stat
      when: not p2pool_version_exists.stat.exists
      tags: [mining, p2pool]

    - name: Fail if P2Pool hash mismatch
      fail:
        msg: |
          P2Pool hash verification failed!
          Expected: {{ p2pool_expected_hash.stdout }}
          Got: {{ p2pool_download_stat.stat.checksum }}
      when:
        - not p2pool_version_exists.stat.exists
        - p2pool_download_stat.stat.checksum != p2pool_expected_hash.stdout
      tags: [mining, p2pool]
    ...
    - name: Clean up P2Pool download files
      file:
        path: "{{ item }}"
        state: absent
      loop:
