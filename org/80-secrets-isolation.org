    ...
    - name: Migrate secrets to .secrets and create symlinks
      block:
        ...
        - name: Migrate and symlink secrets
          ...
          changed_when: "'migrated' in secret_migration.stdout"
...
#+begin_src yaml :tangle ../ansible/fragments/80-secrets-isolation.yml
    ...
    - name: Test sandbox isolation
      # Mirrors the exact mount set of /usr/local/bin/claude-code-sandboxed
      # (org/14-sandbox.org). KEEP IN SYNC with that wrapper — in particular the
      # --bind of the home dir BEFORE the tmpfs overlays. Testing a simpler bwrap
      # invocation here would pass even when the real wrapper leaks secrets.
      shell: |
        if bwrap \
          --ro-bind / / \
          --dev /dev \
          ...
          --bind /tmp /tmp \
          --die-with-parent \
          test -f /home/{{ dev_user }}/.ssh/config 2>&1; then
          echo "not_isolated"
        else
          echo "isolated"
        fi
