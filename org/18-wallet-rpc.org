...
Note on =--disable-rpc-login=: the wallet RPC runs unauthenticated. This
is acceptable only because it binds strictly to =127.0.0.1= — anyone who
can reach it already has local code execution on the box. If the bind
address is ever widened (or another service on this host becomes
attacker-controllable), switch to =--rpc-login user:pass= and store the
...
ExecStart=/usr/local/bin/monero-wallet-rpc \
  ...
  {% if monero_wallet_rpc_restricted | default(true) %}--restricted-rpc \
  {% endif %}--disable-rpc-login \
  --non-interactive \
  --log-file /home/{{ dev_user }}/.bitmonero/logs/wallet-rpc.log \
  --log-level 2 \
  --trusted-daemon \
  --wallet-dir /home/{{ dev_user }}/Monero/wallets
Restart=always
RestartSec=10
Nice=10
PrivateTmp=true
