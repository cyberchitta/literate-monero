...
**Important:** Trezor Suite does NOT support Monero transactions. It's used only for:
- Device firmware updates
- PIN/passphrase management
- General device management

For Monero operations with Trezor, use Feather Wallet (installed above). Feather communicates directly with Trezor hardware for signing Monero transactions.
...
#+begin_src yaml :tangle ../ansible/fragments/07-wallet.yml
    ...
    - name: Get latest Trezor Suite release info
      uri:
        url: https://api.github.com/repos/trezor/trezor-suite/releases/latest
        return_content: yes
      register: trezor_suite_release
      when: not trezor_suite_installed.stat.exists
      tags: [wallet, trezor]

    - name: Find Trezor Suite Linux x64 AppImage
      set_fact:
        trezor_suite_asset: "{{ trezor_suite_release.json.assets | selectattr('name', 'match', '^Trezor-Suite-.*\\.AppImage$') | rejectattr('name', 'search', '-(a|arm|arm64|riscv64)\\.AppImage$') | list | first }}"
      when: not trezor_suite_installed.stat.exists
      tags: [wallet, trezor]

    - name: Download Trezor Suite AppImage
      get_url:
        url: "{{ trezor_suite_asset.browser_download_url }}"
...
**Critical:** Trezor's Monero implementation uses SLIP-0010 key derivation, which is NOT compatible with standard Monero seed formats.

**Monero seed formats:**
- **Legacy:** 25-word seed (original Monero format)
- **Polyseed:** 16-word seed with embedded wallet birthday (used by Cake Wallet, Feather)
- **MyMonero:** 13-word seed

**Trezor uses:** 12/24-word BIP39 seed with SLIP-0010 derivation (incompatible with all above)

**What this means:**
- Trezor recovery seed ONLY works with other Trezor devices
- Cannot convert Trezor seed to any standard Monero seed format (25-word, 16-word Polyseed, or 13-word MyMonero)
- Cannot restore Trezor Monero wallet on non-Trezor hardware (even Ledger with same seed words produces different keys)
- Feather Wallet can access Trezor-derived keys, but only via connected Trezor hardware

**Recovery scenario:** If you lose your Trezor:
1. Buy another Trezor device
2. Restore using your Trezor recovery seed
3. Connect to Feather Wallet - your Monero keys will be accessible again

**Why:** Different hardware wallets chose incompatible key derivation methods (Trezor: SLIP-0010, Ledger: modified BIP-32). This lack of standardization means the same seed words produce different Monero keys on different devices.

**Recommendation:** For maximum portability, use a standard Monero software wallet with Polyseed (16-word) or Legacy (25-word) format. These seeds work across Feather, Cake Wallet, Monero GUI, and other wallets. Use Trezor only if you're committed to the Trezor ecosystem for recovery.

References:
- https://trezor.io/learn/supported-assets/other-cryptocurrencies/monero-xmr-on-trezor
- https://resilience365.com/ledger-trezor-seed-compatibility-monero/
