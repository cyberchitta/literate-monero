...
#+begin_src yaml :tangle ../ansible/fragments/07-wallet.yml
    ...
    - name: Find Trezor Suite Linux x64 AppImage
      set_fact:
        trezor_suite_asset: "{{ trezor_release.json.assets | selectattr('name', 'match', '^Trezor-Suite-.*\\.AppImage$') | rejectattr('name', 'search', '-(a|arm|arm64|riscv64)\\.AppImage$') | list | first }}"
      when: not trezor_suite_installed.stat.exists
      tags: [wallet, trezor]
