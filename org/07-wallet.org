...
#+begin_src yaml :tangle ../ansible/fragments/07-wallet.yml
    ...
    - name: Find Trezor Suite Linux AppImage
      set_fact:
        trezor_suite_asset: "{{ trezor_suite_release.json.assets | selectattr('name', 'match', '^Trezor-Suite-.*\\.AppImage$') | list | first }}"
      when: not trezor_suite_installed.stat.exists
      tags: [wallet, trezor]
    ...
    - name: Get latest Trezor Bridge release info
      uri:
        url: https://api.github.com/repos/trezor/trezord-go/releases/latest
        return_content: yes
      register: trezor_bridge_release
      when: not trezor_suite_installed.stat.exists
      tags: [wallet, trezor]

    - name: Find Trezor Bridge Linux binary
      set_fact:
        trezor_bridge_asset: "{{ trezor_bridge_release.json.assets | selectattr('name', 'match', '^trezord-linux-.*\\.tar\\.gz$') | rejectattr('name', 'search', 'arm') | list | first }}"
      when: not trezor_suite_installed.stat.exists
      tags: [wallet, trezor]

    - name: Download Trezor Bridge
      get_url:
        url: "{{ trezor_bridge_asset.browser_download_url }}"
        dest: /tmp/trezord.tar.gz
        mode: '0644'
      when: not trezor_suite_installed.stat.exists
      tags: [wallet, trezor]

    - name: Extract Trezor Bridge
      unarchive:
        src: /tmp/trezord.tar.gz
        dest: /opt/trezor
        remote_src: yes
        owner: "{{ dev_user }}"
        group: "{{ dev_user }}"
    ...
    - name: Find extracted trezord binary
      find:
        paths: /opt/trezor
        patterns: 'trezord'
        recurse: yes
        file_type: file
      register: trezord_binary
      when: not trezor_suite_installed.stat.exists
      tags: [wallet, trezor]

    - name: Move trezord to standard location
      command:
        cmd: mv "{{ trezord_binary.files[0].path }}" /opt/trezor/trezord
      when:
        - not trezor_suite_installed.stat.exists
        - trezord_binary.files | length > 0
      tags: [wallet, trezor]

    - name: Set trezord executable permissions
      file:
        path: /opt/trezor/trezord
        mode: '0755'
        owner: "{{ dev_user }}"
        group: "{{ dev_user }}"
      tags: [wallet, trezor]
    ...
    - name: Create trezord systemd service
      copy:
        content: |
          [Unit]
          ...
          Type=simple
          User={{ dev_user }}
          ExecStart=/opt/trezor/trezord
          Restart=always

          [Install]
          WantedBy=multi-user.target
        dest: /etc/systemd/system/trezord.service
        mode: '0644'
      tags: [wallet, trezor]
    ...
    - name: Reload systemd for trezord
      systemd:
        daemon_reload: yes
      tags: [wallet, trezor]
    ...
    - name: Clean up Trezor Bridge download
      file:
        path: /tmp/trezord.tar.gz
        state: absent
      tags: [wallet, trezor]
