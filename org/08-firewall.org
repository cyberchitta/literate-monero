...
This phase sets up the firewall:
- UFW (Uncomplicated Firewall) configuration
- Port access control
- P2Pool peer connectivity

SSH rules are managed by WireGuard phase and re-applied via the 'firewall' tag.
...
#+begin_src yaml :tangle ../ansible/fragments/08-firewall.yml
    ...
    - name: Allow service ports
      ...
      loop: "{{ firewall_service_rules | rejectattr('condition', 'defined') | list + (firewall_service_rules | selectattr('condition', 'defined') | selectattr('condition', 'true') | list) }}"
      tags: [security, firewall]
