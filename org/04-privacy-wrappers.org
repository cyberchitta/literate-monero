...
#+begin_src bash :tangle ../configs/templates/oauth-browser.sh.j2
#!/bin/bash
# oauth-browser.sh
# Launch Tor Browser with OAuth provider-specific profile
#
# Usage:
#   oauth-browser.sh <provider> <url> <title-pattern>
#
# Example:
#   oauth-browser.sh github https://github.com GitHub

PROVIDER="${1:-github}"
URL="${2:-https://github.com}"
TITLE_PATTERN="${3:-}"
PROFILE_DIR="$HOME/.tor-browser-profiles/${PROVIDER}-auth"
...
# Get parent firefox PID for this profile
BROWSER_PID=$(ps aux | grep "firefox.real.*profile.*${PROVIDER}-auth" | grep -v grep | grep -v contentproc | awk '{print $2}' | head -1)

if [ -n "$BROWSER_PID" ]; then
    # Browser running
    if [ -n "$TITLE_PATTERN" ]; then
        # Try to find window matching title pattern
        WINDOW=$(hyprctl clients -j | jq -r ".[] | select(.pid == $BROWSER_PID and (.title | test(\"$TITLE_PATTERN\"; \"i\"))) | .address" | head -1)
    fi

    if [ -z "$WINDOW" ]; then
        # No specific match - focus any window from this profile
...
#+begin_src yaml :tangle ../ansible/fragments/04-privacy-wrappers.yml
    # ------------------------------------------------------------------------
    # OAuth Browser Infrastructure
    # ------------------------------------------------------------------------

    - name: Deploy OAuth browser script
      template:
        src: ../configs/templates/oauth-browser.sh.j2
        dest: /usr/local/bin/oauth-browser.sh
        mode: '0755'
      tags: [privacy-wrappers, browser]

    - name: Create app icons directory
      file:
        path: /usr/share/icons/oauth-apps
        state: directory
        mode: '0755'
      tags: [privacy-wrappers, browser]

    - name: Download OAuth provider icon
      get_url:
        url: "{{ oauth_provider.icon_url }}"
        dest: "/usr/share/icons/oauth-apps/{{ oauth_provider.icon_name }}"
        mode: '0644'
      failed_when: false
      tags: [privacy-wrappers, browser]

    - name: Create desktop entry for OAuth browser
      copy:
        content: |
          [Desktop Entry]
          Name=GitHub Auth Browser
          GenericName=GitHub Auth Browser
          Comment=Tor Browser with GitHub OAuth (shared session for GitHub, Element, etc.)
          Exec=/usr/local/bin/oauth-browser.sh {{ oauth_provider.name }} {{ oauth_provider.url }} {{ oauth_provider.title_pattern }}
          Icon=/usr/share/icons/oauth-apps/{{ oauth_provider.icon_name }}
          Terminal=false
          Type=Application
          Categories=Network;Development;
        dest: /usr/share/applications/github-auth-browser.desktop
        mode: '0644'
      tags: [privacy-wrappers, browser]
#+end_src
