...
if BROWSER_PID=$(pgrep -f "profile.*github-auth"); then
    if [ "$MODE" = "own-window" ]; then
        ...
        if [ -n "$WINDOW" ]; then
            ...
            TOR_SKIP_LAUNCH=0 TOR_CONTROL_PORT="" "$TOR_BROWSER" --profile "$PROFILE_DIR" --new-window "$URL" &
        fi
    else
    ...
    # Use bundled Tor, ignore system Tor daemon
    TOR_SKIP_LAUNCH=0 TOR_CONTROL_PORT="" "$TOR_BROWSER" --profile "$PROFILE_DIR" "$URL" &
fi
#+end_src
