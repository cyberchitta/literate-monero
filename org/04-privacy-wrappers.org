...
echo "DEBUG: Tor Browser: $TOR_BROWSER"

mkdir -p "$PROFILE_DIR"

# Configure profile to use bundled Tor on first run
if [ ! -f "$PROFILE_DIR/prefs.js" ]; then
    echo "DEBUG: Creating prefs.js"
    cat > "$PROFILE_DIR/prefs.js" << 'EOF'
// Use bundled Tor, don't try to connect to system Tor
user_pref("extensions.torlauncher.control_port", 9151);
user_pref("extensions.torlauncher.socks_port", 9150);
EOF
else
    echo "DEBUG: prefs.js exists, contents:"
    cat "$PROFILE_DIR/prefs.js"
fi

# Check if Tor Browser with this profile is running
if BROWSER_PID=$(pgrep -f "profile.*github-auth"); then
    echo "DEBUG: Browser already running (PID: $BROWSER_PID)"
    if [ "$MODE" = "own-window" ]; then
        ...
        if [ -n "$WINDOW" ]; then
            echo "DEBUG: Focusing existing window"
            hyprctl dispatch focuswindow "address:$WINDOW"
        else
            echo "DEBUG: Creating new window"
    ...
    echo "DEBUG: Starting new browser instance"
    echo "DEBUG: Command: $TOR_BROWSER --profile $PROFILE_DIR $URL"
