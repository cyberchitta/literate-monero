...
#+begin_src yaml :tangle ../ansible/fragments/80-firewall.yml
    # ------------------------------------------------------------------------
    # Firewall Setup
    # ------------------------------------------------------------------------
    ...
    - name: Allow configured ports (except SSH - handled by WireGuard phase)
      community.general.ufw:
        rule: allow
        port: "{{ item.split('/')[0] }}"
        proto: "{{ item.split('/')[1] }}"
      loop: "{{ firewall_allowed_ports | reject('match', '^22/') | list }}"
      tags: [security, firewall]
