...
# socks5h (not socks5): hostname is resolved through the proxy, so DNS
# queries go via Tor instead of leaking to the local resolver.
[http]
	proxy = socks5h://127.0.0.1:{{ tor_socks_port }}
[https]
	proxy = socks5h://127.0.0.1:{{ tor_socks_port }}
...
#
# ncat with --proxy-dns remote resolves the hostname through Tor; plain
# nc -x resolves it with the local resolver first, leaking the
# destination to the DNS network.

Host github.com
    HostName github.com
    User git
    Port 22
    ProxyCommand ncat --proxy 127.0.0.1:{{ tor_socks_port }} --proxy-type socks5 --proxy-dns remote %h %p
...
#+begin_src yaml :tangle ../ansible/fragments/13-privacy-wrappers.yml
    # ------------------------------------------------------------------------
    # Privacy Wrappers for Applications
    # ------------------------------------------------------------------------

    - name: Install ncat for proxy-DNS SSH transport
    ...
    - name: Display privacy wrappers status
      debug:
        msg:
          ...
          - "  Name: {{ anon_git_name }}"
          - "  Email: {{ anon_git_email }}"
          - "  HTTP/HTTPS proxy: socks5h://127.0.0.1:{{ tor_socks_port }} (remote DNS)"
          - ""
...
# Close the ports however we exit (crash, Ctrl-C, kill) - without the
# trap, a non-zero LocalSend exit would leave the firewall open.
close_ports() {
  sudo ufw delete allow 53317/udp || true
  sudo ufw delete allow 53317/tcp || true
}
trap close_ports EXIT

sudo ufw allow 53317/udp comment "LocalSend transfer"
    ...
    - name: Display LocalSend wrapper status
      debug:
        msg:
          ...
          - ""
          - "Behavior:"
          - "  - Opens firewall ports (53317 TCP/UDP)"
          - "  - Launches LocalSend"
...
Configure domain-based browser routing. Three routing tiers:

1. **KYC domains** → Chromium (kyc-identity profile) - services requiring real identity
2. **Mullvad domains** → Chromium via Mullvad VPN (mullvad-fallback profile) - Tor-blocked sites
3. **Default** → Tor Browser - privacy-first for everything else
...
#+begin_src yaml :tangle ../ansible/fragments/13-privacy-wrappers.yml
    ...
    - name: Configure browser dispatch domains
      copy:
        content: |
          {
            "browser_profiles": {{ browser_profiles | default([]) | to_nice_json }}
          }
    ...
    - name: Display browser dispatch configuration
      debug:
        msg:
          ...
          - ""
          - "Mullvad fallback:"
          - "  - Automatic: Add domains to mullvad-fallback profile"
          - "  - Manual: Use 'Mullvad Browser' desktop launcher"
          - ""
      tags: [privacy-wrappers, browser-dispatch, verify]
#+end_src
...
To add or modify routing domains, edit the =browser_profiles= list in config.yml:
```yaml
browser_profiles:
  - name: kyc-identity
    domains:
...
#+begin_example
# Should open in Chromium kyc-identity profile
/usr/local/bin/browser-dispatch https://chatgpt.com

# Should open in Chromium via Mullvad VPN
/usr/local/bin/browser-dispatch https://cloudflare-blocked-site.com

# Should open in Tor Browser
/usr/local/bin/browser-dispatch https://example.com
#+end_example

*** Manual Mullvad Fallback

When encountering unexpected Tor blocks:

1. Close Tor Browser
2. Click "Mullvad Browser" icon in app menu
3. Navigate to site
4. Close browser when done (auto-disconnects VPN)
5. Optional: Add domain to mullvad-fallback list for future auto-routing
...
All follow the same pattern: configure to use `socks5h://127.0.0.1:{{ tor_socks_port }}` — the `h` variant resolves hostnames through the proxy; plain `socks5` leaks DNS to the local resolver.
