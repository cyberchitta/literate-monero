    ...
    - name: Display browser dispatch configuration
      debug:
        msg:
          ...
          - ""
          - "Profile routing:"
          - "{% for profile in browser_profiles | default([]) %}{% if profile.domains %}  {{ profile.name }}: {{ profile.domains | join(', ') }}{% endif %}{% endfor %}"
          - "{% if not browser_profiles | default([]) %}  (no profiles configured - all traffic to Tor Browser){% endif %}"
          - "  Default: Tor Browser (privacy-first)"
          - ""
          - "Mullvad fallback:"
