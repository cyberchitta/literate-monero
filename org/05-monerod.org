...
- name: Install monerod dependencies
    ...
    - name: Create Monero installation directory
      file:
        path: "{{ monerod_install_dir }}"
        state: directory
        owner: root
        group: root
        mode: '0755'
    ...
    - name: Check if monerod is already installed
      stat:
        path: "{{ monerod_install_dir }}/monerod"
      register: monerod_binary
      tags: [monerod]

    - name: Get Monero release info
      uri:
        url: "https://api.github.com/repos/monero-project/monero/releases/tags/{{ monerod_version }}"
        return_content: yes
      register: monero_release_info
      when: not monerod_binary.stat.exists
      tags: [monerod]

    - name: Find Linux x64 asset
      set_fact:
        monero_asset: "{{ monero_release_info.json.assets | selectattr('name', 'match', '.*linux-x64.*\\.tar\\.bz2$') | list | first }}"
      when: not monerod_binary.stat.exists
      tags: [monerod]

    - name: Download Monero release
      get_url:
        url: "{{ monero_asset.browser_download_url }}"
    ...
    - name: Extract expected hash for downloaded file
      shell: |
        grep "{{ monero_asset.name }}" /tmp/monero-hashes.txt | awk '{print $1}'
    ...
    - name: Fail if hash mismatch
      ...
      when:
        - not monerod_binary.stat.exists
        - download_stat.stat.checksum != expected_hash.stdout
      tags: [monerod]

    - name: Extract Monero release
      unarchive:
    ...
    - name: Create symlink to monerod
      file:
    ...
    - name: Verify monerod binary exists
      stat:
        path: "{{ monerod_install_dir }}/monerod"
      register: monerod_installed
      tags: [monerod]
    ...
    - name: Fail if monerod not found after installation
      ...
      when: not monerod_installed.stat.exists
      tags: [monerod]

    - name: Get monerod version
      command: "{{ monerod_install_dir }}/monerod --version"
      register: monerod_version_output
      changed_when: false
      tags: [monerod, verify]

    - name: Display monerod installation info
      debug:
        msg:
          - "Monerod installed: {{ monerod_install_dir }}/monerod"
          - "Version: {{ monerod_version_output.stdout_lines[0] }}"
          - "Symlinked to: /usr/local/bin/monerod"
      tags: [monerod, verify]
#+end_src
...
**Why automated updates without GPG verification:**
...
2. Size validation (>50% change triggers manual review, detects corruption/attacks)
3. Manual verification available for high-security operators (see Notes section below)

For manual GPG verification: https://github.com/Boog900/monero-ban-list#signatures
