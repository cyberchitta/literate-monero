...
if [ "$OLD_COUNT" -gt 0 ]; then
  DIFF=$(echo "scale=2; ($NEW_COUNT - $OLD_COUNT) / $OLD_COUNT * 100" | bc | sed 's/-.*//' | cut -d. -f1)
  if [ "${DIFF:-0}" -gt 50 ]; then
...
We do NOT verify GPG signatures automatically because:
- GPG keys must be fetched from potentially compromised hosts
- Bootstrapping problem: if GitHub is compromised, keys are too
- No additional security vs HTTPS alone

Instead we rely on:
1. HTTPS transport security (GitHub's TLS)
2. Size validation (>50% change triggers manual review)
3. Manual verification available for paranoid operators
